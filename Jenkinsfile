    ...
    stages {
        ...
        stage('Deploy Postgres to DEV') {
            steps {
                ...
                sh "docker run --name namtrong-postgres --rm -v postgres-data:/var/lib/postgres -e POSTGRES_ROOT_PASSWORD=${POSTGRES_ROOT_LOGIN_PSW} -e POSTGRES_DATABASE=airbnb -p 5432:5432 -d postgres:14.10"
                sh 'sleep 20'
